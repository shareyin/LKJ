...
namespace Pengpai.UI
{
    public partial class ZGraph : UserControl
    {
        ...
        //X轴不需要权值
        private float X_getQuan(float m)
        {
            float quan = 1f;        //临时，权值
            //m = (m < 0) ? -m : m;   //取绝对值
            //if (m == 0)
        ...
        {
            float quan = X_getQuan(m);   //获得该溢出数的权值
            if (isL)
            {
                //如果值是从左边溢出
        ...
        public void _changeXYPointsToNum(float xB, float xE, float yB, float yE,
        ...
        public void ReChange(int X, int Y)
        {
            pictureBoxBigXY.Visible = false;                    //隐藏[波形放大框]
